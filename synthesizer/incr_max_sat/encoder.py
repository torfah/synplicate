...
def dimacs_to_wcnf(wcnf_file, dimacs_path, soft_vars):
    ...
    for line in text_file.readlines():
        ...
        if line.startswith('p cnf'):
            ...
            wcnf_file.write(var)
            wcnf_file.write(" ")
            wcnf_file.write(str(len(soft_vars)))
            wcnf_file.write(" ")
            wcnf_file.write(str(numClauses)) 
...
def encode_base(encoder, output_dir_path, samples, num_of_feature_nodes,feature_partition,feature_weights,label_partition,feature_defs,lower_bound, upper_bound, precision, weights_map, file_name):
    ...
    # # the changing part
    # encoder.phi_threshold(sat_file,lower_bound,upper_bound,precision, lower_bound>0, upper_bound<100)
    ...
    # create dimacs file
    dimacs_path = output_dir_path+"encoding_core_temp.dimacs"
    ...
    return wcnf_path, corr_soft_vars, expl_soft_vars, corr_vars, hard_weight

def encode_threshold(encoder, output_dir_path, lower_bound, upper_bound, precision, file_name):
    sat_file_path = output_dir_path + file_name + "_threshold"
    sat_file = open(sat_file_path, "w")
    sat_file.write("BC1.1\n")

    encoder.phi_dynamic(sat_file, lower_bound, upper_bound, precision)

    sat_file.write("FORMULA := phi_threshold")
    sat_file.write("ASSIGN FORMULA;")
    sat_file.close()

    dimacs_path = output_dir_path+"encoding_thres_temp.dimacs"

    print("Translating to dimacs...")
    os.system(f"./synthesizer/max_sat/bc2cnf -all  {sat_file_path} {dimacs_path}")

    print("Translating to wcnfdimacs...")
    wcnf_path = output_dir_path+file_name+"_threshold.wcnf"

    threshold_vars = encoder.extract_threshold_vars(dimacs_path)
    wcnf_file = open(wcnf_path,"a")
    hard_weight = dimacs_to_wcnf(wcnf_file, dimacs_path, [])
    wcnf_file.close()

    return wcnf_path, threshold_vars, hard_weight
